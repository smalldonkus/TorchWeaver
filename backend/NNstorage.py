...
class NNStorage:
    ...
    def save_network(self, name, json_data, description=None, network_id=None, user_auth0_id=None):
        ...
        cur.execute("""
            INSERT INTO networks (id, name, description, created_at, json_data, user_auth0_id)
            VALUES (?, ?, ?, ?, ?, ?)
        """, (new_id, name, description, datetime.now().isoformat(), json.dumps(json_data), user_auth0_id))
        conn.commit()
        conn.close()
